...
- **CRITICAL — checklist blindness:** AppleScript cannot see native checklist content AT ALL (`body` shows empty `<li>`s, `plaintext` omits the item text; verified 2026-06-12). A note CONTAINING any checklist must be read via the AX/UI path (`notes_ax_read.sh`, or screenshots in a desktop session) and written ONLY via the UI insertion path — a body write would destroy the checklist. Detection: the reliable test is **lines visible on screen/AX that are missing from `plaintext`**. Empty `<li><br></li>` runs in `body` are only a *hint* — a stray blank bullet looks identical, so treat them as "must check via AX", not as proof of a checklist.
...
- **If it refuses on empty `<li>`s, don't just give up — disambiguate.** A stray blank bullet Alex left behind is indistinguishable from a hidden checklist item in body markup alone, so the guard produces false positives (it stranded 3 cards from mid-June to 2026-07-25). Settle it by measurement: compare `notes_ax_read.sh "<note>"` against `plaintext` line-by-line — **any line visible in AX but absent from plaintext = a real checklist → use 5b**; identical line sets = nothing is hidden, so re-run the write as `NOTES_VERIFIED_PLAINTEXT_LEN=<plaintext length> notes_set_body.sh …`. Compare normalised *line sets*, not byte counts (AX returns UTF-8 bytes, `plaintext` counts characters — em-dashes and emoji make those differ harmlessly). If the GUI is unavailable, you cannot disambiguate: leave the card queued.
