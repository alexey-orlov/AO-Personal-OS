...
- `softserve/jumpstart-pm` — DESIGN of the Jumpstart agentic-AI enablement offering and its PM-focused extension: Activation Pod, program tiers (Full / Express / Light), maturity assessment/heatmap, AI governance, pipeline + sales calls for client engagements like DAX / GI Partners / Telenor / Commerce IQ / nContracts, counterparts Inna Abolikhina / Bogdan.
- `softserve/payworks-jumpstart` — DELIVERY of the AI PM Jumpstart to the client **Payworks** (.NET legacy modernization; the product stream of the "Stream Tools" engagement): delivery planning and internal working sessions, client sessions/workshops, demo Team OS repo and tooling/access setup, counterparts David / Roberto Bautista / Marlene / Vladislav. A call about Payworks specifically goes here, not to `jumpstart-pm`.
