...
- **Companion reference docs (reconcile, don't just fold the headline)**: an area/subproject often has durable reference docs beside its README/subproject page (e.g. `softserve/oracle-team.md`, `softserve/oracle-ai-offerings.md`, `job-search/positioning.md`). When a new artifact carries a fact that **extends or contradicts** one of these — a stakeholder's role/seniority/ownership/decision-power changes, a product or offering detail shifts, a positioning angle moves — you MUST reconcile that doc too, not only fold the headline into the subproject page + Now snapshot (a headline in index.md while the detailed doc stays stale leaves the wiki self-contradictory). Preserve the doc's `_source`/`(inferred)` labels, bump its `_updated:` date + add the new provenance. Find these docs by scanning the area dir for non-`README` `.md` pages whose subject the artifact's facts touch — check them on every fold, not just when a subproject page exists.
  - **Reconcile the structure, not just the cell.** Before editing, ask whether the new fact invalidates the doc's *organizing scheme* — its headline thesis, section taxonomy, table grouping, or where a person/item is placed — not merely the value in one row. If a person now spans/leads categories the doc splits them into, MOVE them to the right place (e.g. lift them above the split into a cross-cutting "lead" entry) and fix the headline; don't append "also X" inside the now-wrong slot. Patching the cell while leaving a contradicted schema is the same staleness failure as not editing at all. Read the artifact's full source phrasing and carry its actual breadth — don't narrow an explicit claim ("leads the whole engagement across data & AI") down to fit the existing structure ("is also in the other bucket").
...
| Training log | a photo of a handwritten gym-notebook page (dated header, exercise lines with kg + `3x10`-style sets), or a bare weigh-in | INVOKE the `gym-log` skill with the attachment path — it writes the strength block to the "My training" sheet and delivers a 🏋️ digest. **Never file a training page as an Insight**: an Apple-Notes one-liner loses the numbers and the session never reaches the sheet. No credentials in the run → the skill queues the payload to `automations/gym-log/pending/` (a queued payload is NOT a logged session — say so in the summary) |
...
- Companion reference docs reconciled (step 3): if the artifact changed a stakeholder/product/positioning fact, the durable doc that holds it (`oracle-team.md`, `oracle-ai-offerings.md`, `positioning.md`, …) was rewritten to match — not left stale while only the Now snapshot moved — AND its structure (headline/taxonomy/placement) was re-checked, not just a single row patched inside a now-wrong scheme.
