...
- `context/` — the knowledge tree, organized by AREA. `index.md` (map + "Now" snapshot). `areas/<area>/` per area — `README.md` (live-state wiki page), optional `<subproject>.md` pages, `calls/[<sub-context>/]` (pipeline-routed call notes; taxonomy owned by Axis 2 of `.claude/skills/classify/SKILL.md`), `docs/` (manually added source materials Alex chose to commit). Areas: `softserve`, `gigacloud`, `job-search`, `laba`, `personal-os`, `other` (classifier catch-all). Cross-area: `people/` (person pages), `book-shortlist.md`, `_meta/processed.txt` (ingestion ledger), `_inbox/` (cloud Drop Zone staging — committed; folded daily by the cloud routine, processed drops archived in `_inbox/processed/`), `knowledge/` (`notes/` themes + `explore-queue.md` owned by `context-update`; `podcasts/` owned by `/podcast-insights`).
- `inbox/` — LOCAL drop zone for files that must never reach GitHub (git-ignored except its README; folded by local `/context-update` sweeps only). The primary Drop Zone path is cloud: Telegram 📥 Drop Zone → n8n → `context/_inbox/`.
