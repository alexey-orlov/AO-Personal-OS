...
- NEVER re-enable `telegram@claude-plugins-official` (user settings keep it `false`). Telegram allows ONE getUpdates consumer per bot; the upstream plugin's server steals that slot from the chat bridge and silently eats Alex's messages (2026-06-12 incident). The only enabled telegram plugin is this repo's gated fork `telegram@ao-personal-os` (`automations/telegram-chat/plugin/`): it polls only when `TELEGRAM_CHANNEL_POLL=1`, which only the bridge's `run.sh` exports — every other session is tools-only by design. See `automations/telegram-chat/README.md` "Single-poller invariant" before touching any of this.
...
- `automations/apple-notes-sync/` — launchd agent (every 30 min, this Mac): files queued goal/task/insight cards from `context/_inbox/apple-notes/` into Alex's pinned Apple Notes (`_ToDo` folder; bullets with `📥` suffix, insert-only, backed up before every write) and refreshes the read-only note snapshots under `context/areas/<area>/apple-notes/`. Engine = the `apple-notes-sync` skill; Apple Notes is Mac-only, hence local. See its `README.md`.
