...
- `automations/podcast-streaming/` — daily YouTube podcast digest. Runs in n8n cloud (workflow "Podcast streaming v3 (resilient)"), not on this machine; the folder holds the exported workflow JSON + ops doc. Edit via the n8n MCP, then re-export the JSON. See its `README.md`.
