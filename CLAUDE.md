...
- `automations/second-brain-delivery/` — n8n workflow "Second-brain delivery (cloud)": flushes queued Telegram notifications from `context/_inbox/outbox/`, posts each goal/task in `context/knowledge/goals-tasks.md` as its own 🎯/☑️ message in the Goals & Tasks topic (stamping ` · tg:<id>` back into the line), and handles Alex's 👍 reactions — like = done, un-like = reopen (via the capture workflow's reaction branch). See its `README.md`.
