...
- NEVER re-enable `telegram@claude-plugins-official` at user scope (`~/.claude/settings.json` keeps it `false`), and never call `mcp__plugin_telegram_telegram__*` tools from a non-bridge session. Telegram allows ONE getUpdates consumer; an upstream plugin instance in any other session kills the bridge's poller and silently eats Alex's messages (2026-06-12 incident). The bridge force-loads the plugin via `--channels`; the poll gate (`TELEGRAM_CHANNEL_POLL=1`) + watchdog live in `automations/telegram-chat/` — see its README "Single-poller invariant".
