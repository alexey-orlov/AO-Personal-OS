...
`classify` returns three lines — `type:` (picks the analysis skill + filename), `folder:` (the meeting-context subfolder under `outputs/call-notes/`), and `coaching:` (yes/no — see the gate section above). The folder dimension is orthogonal to type. Valid folders:
...
- **Token expiry — Testing-mode trap.** If the OAuth client is still in "Testing" publishing status in Google Cloud Console, the refresh token is invalidated after 7 days, and runs start failing with `invalid_grant: Bad Request`. Fix in 30s: delete `.work/calendar/token.json`, then run a tiny snippet that re-runs the consent flow — `source config.sh && .work/venv/bin/python3 -c "from google_auth_oauthlib.flow import InstalledAppFlow; from pathlib import Path; import os; s=['https://www.googleapis.com/auth/calendar.readonly']; c=InstalledAppFlow.from_client_secrets_file(os.environ['CALENDAR_CREDS'], s).run_local_server(port=0); Path(os.environ['CALENDAR_TOKEN']).write_text(c.to_json())"` — and then backfill the missed note's calendar header by running `calendar_lookup.py <iso-recording-ts> /tmp/h /tmp/c` and pasting the result into the note. Permanent fix: publish the OAuth app to "In production" in Google Cloud Console (calendar.readonly is restricted but does not need verification for a single internal user).
