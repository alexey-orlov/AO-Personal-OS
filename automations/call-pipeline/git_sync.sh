#!/usr/bin/env bash
# git_sync.sh <commit-message> <file> [file ...] — stage, commit, push file(s).
...
msg="${1:?usage: git_sync.sh <commit-message> <file> [file ...]}"; shift
[ $# -gt 0 ] || { echo "[git] no files passed; skipping" >&2; exit 0; }
...
if git push >/dev/null 2>&1; then
  echo "[git] pushed: $(for f in "$@"; do basename "$f"; done | paste -sd, -)"
else
