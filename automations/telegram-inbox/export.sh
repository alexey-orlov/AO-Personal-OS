#!/usr/bin/env bash
# Re-export the live "Drop Zone capture (cloud)" n8n workflow to workflow-dropzone-capture.json,
# stripping everything that must never reach the repo (CLAUDE.md hard rules):
#   - the Telegram group chat id      → REDACTED_GROUP_CHAT_ID
#   - the trigger webhookId (live URL) → REDACTED_WEBHOOK_ID
...
# webhookId is the live trigger URL path — never commit it
for n in out['nodes']:
    if 'webhookId' in n:
...
open('workflow-dropzone-capture.json', 'w').write(s + '\n')
