...
- **Contents** (top folder → wiki home):
  - `Projects/Oracle/` → [oracle](../areas/softserve/oracle.md) — the partnership working set. **Reorganized ~2026-09-10 into `Customers/` + `Packs/`** (paths recorded before that date are stale):
    - `Projects/Oracle/Customers/<name>/` — per-customer working sets: `Belron/`, `Bosch/`, `DHL/` (the `UC #6 DHL Client Compass` SoW), `KPN/`, `NATO/` (→ [oracle-defense](../areas/softserve/oracle-defense.md) — `FreeTech_UseCases_OnePager.pptx`, the two NVIDIA Nemotron decks from Karsten's 2026-08-28 thread (`NVIDIA Nemotron - Master Deck.pdf`, `NVIDIA Agent Toolkit - Customer Deck.pdf`), the NATO adaptation xlsx + brief html of 2026-09-06, and the map decks `NATO AI usecase map*.pptx` — `_upd.pptx` is **Alex's hand-edited working copy**, read it before matching anything to the deck — plus the review deck `NATO AI usecase map - pipeline cases (review) 2026-09-11.pptx` with its two previews `… - slide 1 map.png` / `… - slide 2 pipeline.png`, delivered 2026-09-11), `NHS/`, `RiyahdAir/`, `SBG/` (→ [sbg-poc](../areas/softserve/sbg-poc.md) — the `UC #5.1 …` SOW + the `SBG_AI-Q_PoC_WBS_…` workbook). `AIDP/` still sits at the `Projects/Oracle/` root.
    - `Projects/Oracle/Packs/` → [oracle-packs](../areas/softserve/oracle-packs.md) — the productization stream: `Oracle packages.xlsx` (the packaging tracker), `Workforce optimization package/`, `Large Document Extraction and review package/`, `Account Insights/` (the AI Signal-Impact Engine one-pager), `AI Lakehouse quick start/`, and `Use case maps/` (the **productization pipeline spreadsheet** `SoftServe-NVIDIA-Productization Use cases-Pipeline…xlsx` + the **agentic pattern taxonomy** `AI workflow patterns - AIDP-NVIDIA-OracleAI mapping.xlsx` — v2 tab, red-team report and row-writer prompt added 2026-09-07). Since 2026-09-24 a pack's **final** artifacts are delivered to `Oracle AI & Data Solutions/` (below); this folder keeps the earlier copies, the older one-pagers and decks, and the Repair-or-replace working material.
    - `General/` (GTM deck, partnership-vision drafts, the EMEA Business Alignment deck + its use-case-map v2 copy); `Presentation.pptx`.
  - `Oracle AI & Data Solutions/` (OneDrive **root**) → [oracle-packs](../areas/softserve/oracle-packs.md) — the **final pack artifacts and nothing else**, one folder per pack (`Account Insights/`, `Workforce optimization package/`, `Large Document Extraction and review package/`, `Repair-or-replace decisions/`): the files the mini-site's sales-kit email links to. Working files and drafts never go here; they stay in `~/oracle-packs/<slug>/` (Alex, 2026-09-24). **Sharing gotcha:** SoftServe's tenant allows no *Anyone* link on this OneDrive (sharing API on 2026-09-24: anonymous read links disabled by policy; *People in SoftServe* and *Specific people* links allowed), so no link from here opens for an Oracle seller. Added 2026-09-24.
  ...
  - `Monthly AI products overviews/` (OneDrive **root**) → area-level — the monthly R&D solutions reviews, incl. Vlad's `Agentic AI for SW _ September 2026.pptx`. Its `AI Solutions review - Sep/` subfolder holds the **"AI Days / AI Solutions Playbook · September 2026"** master deck `SoftServe AI Repeatable IP-Based Customer Stories.pptx` (450 MB — one block per solution) plus Alex's two section blocks: `Oracle AI Packages - section slides.pptx` (→ [oracle-packs](../areas/softserve/oracle-packs.md)) and `Agentic OS for Knowledge Workers - block draft 2026-09-11.pptx` (→ [jumpstart-pm](../areas/softserve/jumpstart-pm.md)). Added 2026-09-11.
  - Other root-level context beyond `Projects/`: `Meetings/`, `Recordings/`, `Hiring/`, `Productization - General/` (`Customer projects.xlsx`), `Attachments/`, Teams / Copilot chat files, `Agentic Engineering.xlsx`.
