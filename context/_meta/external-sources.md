...
- **Contents** (top folder → wiki home):
  - `Projects/Oracle/` → [oracle](../areas/softserve/oracle.md) — the partnership working set. **Reorganized ~2026-09-10 into `Customers/` + `Packs/`** (paths recorded before that date are stale):
    - `Projects/Oracle/Customers/<name>/` — per-customer working sets: `Belron/`, `Bosch/`, `DHL/` (the `UC #6 DHL Client Compass` SoW), `KPN/`, `NATO/` (→ [oracle-defense](../areas/softserve/oracle-defense.md) — `FreeTech_UseCases_OnePager.pptx`, the two NVIDIA Nemotron decks from Karsten's 2026-08-28 thread (`NVIDIA Nemotron - Master Deck.pdf`, `NVIDIA Agent Toolkit - Customer Deck.pdf`), the NATO adaptation xlsx + brief html of 2026-09-06, and the map decks `NATO AI usecase map*.pptx` — `_upd.pptx` is **Alex's hand-edited working copy**, read it before matching anything to the deck), `NHS/`, `RiyahdAir/`, `SBG/` (→ [sbg-poc](../areas/softserve/sbg-poc.md) — the `UC #5.1 …` SOW + the `SBG_AI-Q_PoC_WBS_…` workbook). `AIDP/` still sits at the `Projects/Oracle/` root.
