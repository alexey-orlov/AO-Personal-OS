...
- **Contents** (top folder → wiki home):
  - `Projects/Oracle/` → [oracle](../areas/softserve/oracle.md) — the partnership working set. Per-customer subfolders (`AIDP/`, `Belron/`, `Bosch/`, `DHL/`, `KPN/`, `NHS/`, `RiyahdAir/`, `SBG/`); `General/` (GTM deck, the partnership-vision deck drafts, pipeline review copies); `Use case maps/` (the canonical **productization pipeline spreadsheet** `SoftServe-NVIDIA-Productization Use cases-Pipeline…xlsx` + the **agentic pattern taxonomy** `AI workflow patterns - AIDP-NVIDIA-OracleAI mapping.xlsx`; the NATO adaptation xlsx delivered 2026-09-03 belongs here); `Workforce optimization package/` (WfO service-packages deck, sales one-pager, accelerator-pack one-pager); **`NATO/`** → [oracle-defense](../areas/softserve/oracle-defense.md) — the **two NVIDIA Nemotron decks (Public & Defense EMEA)** received via Karsten's 2026-08-28 thread (registered 2026-09-06; not yet read from a cloud session); `KPIs.xlsx`; `Presentation.pptx`.
