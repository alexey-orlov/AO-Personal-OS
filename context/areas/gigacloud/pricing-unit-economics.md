...
- **Presentation layer (⚠ lettering differs from this page's classes):** the stakeholder action-plan deck (`~/Downloads/GigaCloud_Product Margin Plan_27-AUG-2026.pptx`, 11 slides, UA, GigaCloud template) deliberately simplifies to **4 models per Alex's brief — A = direct FTE (Delivery/Support), B = specific products (Sales/CAC), C = product groups (ВОК → Resell), D = general** — which matches neither the Models-sheet block labels in `Margin.xlsx` (its "Model C" block = deck B; its "Model B" block = deck D) nor the v4 classes A–G in [pricing-cost-allocation-approach.md](pricing-cost-allocation-approach.md). When talking to Alex about "модель B/C/D", the deck lettering is canonical. Models grouped on the slide as A → COGS · B+C → витрати на збут · D → general (v2, 2026-08-28). **v3 (2026-08-28): A split into A1 (FTE → ФОТ) + A2 (direct supplier purchases: units included × unit price → COGS)**; A2 worked example = Cubbit S3 Storage 10 TB from CRM — 10 000 GB-юнітів × 0,35 ₴/GB/міс = 3 500 ₴ ≈ €72 COGS vs €85 price → ~15% margin (FX 48,5 illustrative; ⚠ CRM stores the reciprocal 0,0001 in "Supplier units included" — data-quality note). Alex hand-edits the deck file directly — always read the current file from Downloads before touching it. (chat, 2026-08-27/-28)
- **Product→component bridge — ALEX'S DECISION (deck v4, 2026-08-28, supersedes the v2 price-share choice):** product/group-level attribution (models B, C, D) reaches components **by the average weight of the component's COGS in the average product configuration**; a separate analysis will decide whether average configurations must be computed per client segment (Enterprise vs smaller clients). Alex chose COGS-weight over the price-share base Claude recommended — rationale: prices are themselves being redesigned (unstable anchor), COGS is price-independent. Consequence: the COGS data fixes (187 zero-price rows, catalog↔P&L reconciliation) are now on the critical path of the bridge, not just of floors. ⚠ Note this diverges from the v4 doc's EPMU/price-share spread — reconcile when the rate card is computed. The identical note sits on the models slide + B/C/D example slides. (chat, 2026-08-28)
...
- **Stakeholder alignment.** Action-plan deck built and iterated to v5 (13 slides: мета/результати + a second «Мета» slide with the **output-table schema** — per-component Компонент/COGS/CAC/Загальні/Поточна ціна/Прибуток зараз ₴ і %/Мін. прибутковість/Нова ціна/Зміна %/Мін. ціна продажу, headers color-coded обчислюємо-red / вже пораховано-grey (COGS) / фіксовано-ltgrey (поточна ціна) / політичне рішення-ink (мін. прибутковість), two examples: +11% і −33% · price-stack стовпчик у 2 версіях · 6 principles · models A1/A2/B/C/D + worked examples (A2 = Cubbit) · P&L-restructuring slide (InfoSec 2.1.3 → split B + D) · next steps in 3 phases, 10 steps — step 10 = **мінімальна прибутковість, погоджують CFO + CEO + CBDO** («в нуль» не продаємо); мін. ціна продажу = loaded cost ÷ (1 − мін. приб.), причому мін. прибутковість — % від фактичної ціни продажу (не прайсової). Model C example: ВОК 343 571 ₴/міс → Resell group → +3,45%, share assumed 100% until FTE survey. → next: present, then департамент-alignment. Owner: Mine. (chat, 2026-08-27/-28)
