...
- **Product→component bridge (deck v2 decision, 2026-08-28):** product/group-level costs (models B, C) reach components **pro-rata to the component's share of realized price** (averaged over billing for the rate card) — consistent with v4's EPMU spread. Margin rejected (circular: attribution changes margin; punishes high-margin components), COGS rejected (incomplete data — 187 zero-price rows; over-loads resell whose COGS ≈ 95% of price). Price is the only complete axis (all 463 components priced), sums back to the pool exactly, discounts self-adjust. (chat, 2026-08-28)
