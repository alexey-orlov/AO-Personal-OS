...
.srcs{font-size:12px;color:var(--ink-3);margin-top:5px}
.srcs a{color:var(--accent);text-decoration:none;font-weight:560}
...
<div class="wrap">
  <header>
    <div>
      ...
      <div class="srcs">Data: <a href="https://docs.google.com/spreadsheets/d/1EaibAz0BCISjOyYDsIJCx6v_q6jW6cFDz-mD2dUxOYc/edit"
        target="_blank" rel="noopener">the full ranking table (Google Sheet)</a> — every figure
        on this map, with sources per row.</div>
    </div>
    <div class="stats">
...
// Pinning happens on pointerup (see the drag block): pointer capture retargets the click
// event to the svg element itself, so a click handler here would never see the place.
...
svg.addEventListener('pointerdown', e=>{
  drag={x:e.clientX, y:e.clientY, vx:vb.x, vy:vb.y, moved:false,
        target: e.target.closest ? e.target.closest('.place') : null};
...
svg.addEventListener('pointermove', e=>{
  ...
  if(!drag.moved) return;                       // a still press is a click, not a pan
  const r = svg.getBoundingClientRect();
