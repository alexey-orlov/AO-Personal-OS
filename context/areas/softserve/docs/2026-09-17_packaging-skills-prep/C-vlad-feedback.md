...
- The 2026-07-24 sync is now **fully transcribed** (31 min, all of it). §1, §2's 07-24 block, §3 and §6 rewritten against it; §4, §5 and §2's later blocks are unchanged except where noted below. The sources list gains the full transcript and the call note, and its stale scratchpad path for the partial gloss is corrected to the committed one.
...
Reconstructed as a numbered procedure. Steps **4 → 4a → 5** are his explicit **"трёхходовка"** ("three-step process", his word), step 3 is the gap task he appended to the front of it, and the rest is the surrounding scaffolding from the same call and from his own artifacts. Top-level numbers are stable — §5 refers to "step 10" and "step 11"; everything added from the full transcript is letter-suffixed.

> **Corrected 2026-09-18.** The partial gloss made steps 4–6 look like *differentiate → widen → adjudicate*. The full transcript shows the three moves are **validate the scenario set → challenge the step list against it → generate per-step differences**, in that order, each as its own prompt. Getting the order wrong matters: steps 1 and 2 exist to make the scenario set trustworthy *before* anything is differentiated against it.
...
2. **Enumerate the workflow as N steps** (Vlad's document case ran to 10) and, for each step, fill the three lenses: what the vendor pack provided · what we implemented · what stays custom per engagement (R8). Mechanics from the full transcript:
   - Derive the steps from the **delivered workflow diagram, reviewed side by side with the table**, until every delivered pass has a row and every row has a provenance; order rows in flow order (R7g).
   - Give every block a **plain-language row for its core job** before its scoring / coverage / confidence rows (R7f).
   - **Split** where extension paths diverge, **merge** where the task is identical, and make every distinction name its downstream consequence (R7h).
   - Name features at **task altitude**, with no model or component names (R7d).
   - Read every custom-work cell back as *"the feature is perfect; we arrive at a client; the custom work is ___"* (R7e).
   - Record status **from the delivery artifacts**; flag anything you have not seen as an assumption plus a verification task (R7m).
2a. **Name the non-universal constraints the delivery rested on** — one document type, one language, one nomenclature, one integration target — and carry each into the table as a variability axis. This is the exercise's actual object (R7a).
3. **Up-front vendor-taxonomy gap task** (run this BEFORE the three-step process, as its own prompt): *"I see it as these steps; look at such-and-such vendors and, based on how they structure their set of capabilities, highlight which steps I'm missing or where there are clearly other tasks."* Expect noise; harvest only the gaps (R5). Reconcile names with the vendor vocabulary, and turn every term you cannot define into a numbered research question with a method attached — then make the table cover it (R1).
4. **Three-step process, move 1 — validate the scenario set.** *"I see 10 steps, I see 4 principally different vertical cases — validate; maybe I'm missing an important fifth one."* Alex's own illustration of a good answer: *"in all your cases everything goes through validation; there could be a scenario where no validation is possible at all — in insurance."* A scenario qualifies only if its **entities** differ, not its industry label (R3).
4a. **Move 2 — challenge the step list against the validated scenarios.** *"Now here are my 10 steps: challenge them against these 5, 6, 7 patterns we found. Are the steps exactly the same everywhere, or am I missing one?"* Output is a corrected step list, not yet any differences.
5. **Move 3 — generate the per-step differences across the scenarios**, seeded with hand-written examples so the model has the altitude: *"here are the N steps; I see examples of differences like this — in contracts with vendors this matters, here it doesn't — develop the theme and think where else there may be differences."* One **"what matters here"** cell per step × scenario (R3).
...
7. **Interrogate every step for its failure path** — what happens when the expected thing isn't there? Probe it with a deliberately out-of-domain input ("what if you put a restaurant menu in?"). No handling = an explicit "not covered" row and an OUT OF SCOPE line; a prediction about what *would* happen is a question, not a status (R2).
7a. **Run the nuance pass on each step:** restate it as the abstract task, ask *"what else can there be?"*, and answer with concrete variants (typo · synonym · wrong format) rather than categories. Express what the client must be able to configure, never the mechanism ("a confidence score is already an implementation method"). Each un-built answer is an empty-circle row (R7i).
...
9a. **Evidence pass.** Demote anything untested to a roadmap row, however plausible ("the LLM can theoretically do it" is not a selling point) (R7j); re-decide every boundary line as *pack* scope rather than inheriting the PoC's out-of-scope list (R7k); keep the split "standard import/export API = product, each concrete integration = custom".
9b. **Completeness gate — run before declaring the table done.** Are there rows that are definitely *not* built? If every row is ● or ◐, the list was reverse-engineered from the delivery and step 2a has not happened (R7c). Then the honesty check on the other side: is the envisioned scope complete enough that a prospect recognizes their own X — and bounded enough that we are not "making soup out of a ball"? (R7b)
...
11. **Derive the external artifacts** on the fixed one-pager anatomy (R10, R11), with the S/M/L tier table on integration depth (R21–R23) and every figure caveated (R13). Keep features and architecture layers in separate blocks (R11b); do not let the packaging tagline become the one-liner (R11a). Anchor the integration point and the ICP to the partner's concrete product even where the build is product-agnostic, and hunt for that anchor deliberately when it isn't obvious (R7l).
...
**CLOSED 2026-09-18 — the 2026-07-24 sync.** The previously-missing 26 minutes (0:35–27:02) were the top ask on this list and are now transcribed in full off the Teams recording. They were, as expected, the stretch where Alex walks the 10-step table row by row — and they carried **13 rules that the partial gloss did not contain at all** (R7a–R7m), including the exercise's stated rationale (R7b), its cheapest quality gate (R7c) and the systematic mis-reading of the custom-work column (R7e). The ordering of his "three-step process" also turned out to be different from the reconstruction (see the correction note in §3). *Lesson for the remaining asks: the reconstruction-from-artifacts method underestimated the coaching corpus by roughly twice — the other exports are worth more than this file previously implied.*

**What the full transcript still does not give:**
- **The two on-screen artifacts are never read out.** Vlad's table is only sampled aloud (a handful of row names: `native multilanguage`, `page routing`, `confidence scoring`, `coverage`, use-case-boundary items), and the delivered "purple scheme" is referenced only by pass number (4a / 4b / 4c). **Resolve from the artifacts, not the audio:** the table → `[Oracle Packages] Large Document Extraction and Validation - Acceleration Pack One-pager.pdf` (2026-07-27, the same table three days later); the diagram → `NEW_Architecture Vision - Riyadh Air v3.docx` §8 (see §5).
- **Whose taxonomy was on screen.** Alex reconciles our steps against a vendor capability list containing *human-in-the-loop exception handling · decisioning · validation* and a maturity axis (*Full Automation · Sub-Human [likely "Super-Human"] Automation · Product Hardening · Enterprise Scale*). He names **HyperScience** as an inspiration for the exceptions concept, but never says whose list is on screen. R1 and R5 would both be sharper with the source. **Ask Alex which vendor taxonomy he had open on 2026-07-24.**
- **Whether the assigned work landed.** Four deliverables were set in this call — the table↔diagram mapping, the *validation* / *decisioning* research, the three-step prompting run, and the search for an Oracle anchor system at the document case's data-source end. Nothing captured anywhere shows their output or Alex's reaction to it. The Jul-27 one-pager is the only downstream evidence, and it is three days later.

**Syncs still with no transcript in this repo:**
1. **Every other Alex↔Vlad sync.** The 07-24 transcript confirms a recurring cadence explicitly — *"как и в прошлый раз"* (a prior sync, where the same "no unbuilt rows" finding was already raised), *"давай уже постараемся в следующий раз точно таблицу эту добить"*, *"мы посмотрим на эту систему на следующий раз"* — but **no sync before or after 2026-07-24 appears anywhere**: not in `calls/oracle/`, not in `calls/jumpstart-pm/`, not in the local session transcripts. **Now the highest-value export**, and note there is at least one *earlier* one too, not only later ones.
...
4. **`Payworks — PM Jumpstart program — sync` (190 MB, 2026-07-27)** — OneDrive `Recordings/`. The 07-24 sync deferred the whole Jumpstart discussion to "next time" and this is the nearest Payworks meeting to it; it is also where Vlad's discovery→skills conversion would have been reviewed.
5. **`Current Team's projects scope for potential productization-20260701_113802-Meeting Recording.mp4`** — OneDrive `Recordings/`, **414 MB**. This one *is* covered by the 2026-07-01 call note; no export needed.
6. **`Large Document Extraction and Validation Demo.mp4`** — OneDrive `Customers/RiyahdAir/`, **159 MB**, 2026-09-15. Unnarrated per the wiki, but worth confirming.

**Full OneDrive recording inventory (so Alex can pick):** the only Oracle/Vlad-relevant items in `OneDrive-SoftServe,Inc/Recordings/` are the Butenko onboarding (609 KB, 2026-07-06), *Current Team's projects scope for potential productization* (414 MB, 2026-07-01), *Oleg / Alex sync* (294 MB, 2026-06-23), three *Oracle partnership next steps — Neil meeting prep* fragments (2026-07-11), *Payworks — PM Jumpstart program — sync* (190 MB, 2026-07-27) and *SBG discussion* (65 MB, 2026-08-20); plus *T-Shirt Packaging approach discussion* (304 MB, 2026-06-12) directly under `Projects/Oracle/`. **No recording of any other Alex↔Vlad packaging sync is on this disk** — they are SharePoint-only. (The 2026-07-24 recording reached `~/Downloads/` by hand, which is how it got transcribed; the same route works for the rest.)

**Ask Alex to export, in priority order:**
1. Every other "Oracle Productization & BA JumpStart Sync" occurrence — **the one before 2026-07-24** (where the "no unbuilt rows" finding was first raised) and the 2026-08-xx / 2026-09-xx instances, where the Account Insights pack and the Sep-10 artifacts were reviewed.
2. The 2026-07-27 Payworks PM Jumpstart sync.
...
- **Which vendor's capability taxonomy was on screen on 2026-07-24** (see above) — it is the concrete instance behind R1/R5.
- **`context/areas/softserve/oracle-team.md` has no Vlad entry at all**, and `oracle-packs.md` says *"Vlad (inferred: Vlad Selyotkin or Vladyslav Butenko — **not disambiguated in the tracker**)"*. The full transcript **settles it for the packaging work**: the same person owns the Oracle pack tables and the Payworks Jumpstart discovery in one call, which matches the 2026-09-08 *"the report Vlad prepared"* (Payworks) and the 2026-09-10 *"additional files from Vlad"* (both packs) — **Butenko**. Fix the wiki; the only thing still unconfirmed is who owns the pack *demo videos*.
...
- **Riyadh Air was parked as unpackageable by Gero on 2026-06-26 and became pack 2 anyway.** What changed? The answer is a criterion the go/no-go skill needs. (The 07-24 transcript shows Alex working the case as a pack without revisiting the question.)
...
- `/Users/olekorlov/Documents/GitHub/AO-Personal-OS/automations/call-pipeline/.work/transcripts/Oracle Productization and BA JumpStart Sync-20260724_133119-Meeting Recording.txt` — **the full 2026-07-24 transcript** (git-ignored; source recording `~/Downloads/Oracle Productization & BA JumpStart Sync-20260724_133119-Meeting Recording.mp4`)
- `/Users/olekorlov/Documents/GitHub/AO-Personal-OS/context/areas/softserve/calls/oracle/2026-07-24_133119_one-on-one_vlad-productization-sync.md` — the call note written from it
- `/Users/olekorlov/Documents/GitHub/AO-Personal-OS/context/areas/softserve/docs/2026-09-17_packaging-skills-prep/vlad-2026-07-24-transcript-partial.md` — Alex's earlier partial gloss (superseded; kept for the two timestamped anchors)
