...
- **AIDP — Oracle AI Data Platform:** Oracle's broad, foundational data platform — unifies structured / unstructured / batch / real-time enterprise data into one AI-ready estate; **lakehouse architecture** (Apache Iceberg / Delta, medallion bronze→silver→gold) combining OCI + Autonomous AI Database + OCI GenAI. This is the "Databricks/Snowflake answer" the wiki has tracked since the 2026-06-12 AIDP call (differentiation = 20–60% TCO, forthcoming semantic-layer SKU, zero-copy catalog-of-catalogs, multi-cloud). Oracle projects **100s of qualified AIDP leads** from the Sept events. **Product-state corrections from the 2026-09-07 pattern-map red-team (web-verified by its agents; re-check before quoting):** AIDP Workbench notebooks and Workflows (scheduled pipelines) are GA, while **Experiments / Models — the MLOps registry — still carried a Preview label in the docs as of 2026-09** (an Oracle blog of 2026-07-31 calls MLOps GA; treat as GA-pending); **AI Agents / Agent Flows** (SQL / RAG / prompt / HTTP / MCP tools) are the Workbench's agent layer (Oracle's GA blog also says "Agent Studio"; "AI Agent Studio for Fusion Applications" is a separate Fusion product); the Jun-12 "no dedicated Databricks migration tooling" answer is outdated — **oracle-samples/oracle-aidp-samples ships an MIT-licensed sample Databricks → AIDP migration toolkit** (plugin v0.2.0; sample code, not Oracle-Support-backed); and the **OCI Anomaly Detection managed service is retired** (deprecated 2024-03-07) — Oracle's anomaly-detection path is Spark / OML models over the lakehouse, not a prebuilt AI service. (chat, 2026-09-07)
...
- **Oracle Data Science Agent is shipping** — a conversational-ML feature of Autonomous AI Database 26ai (profile / feature / train / score by conversation; ADB Serverless release note 2026-06-30, no explicit GA label), no longer "demo-stage"; the other 26ai agentic / governance pieces the red-team verified are **Oracle AI Database Private Agent Factory** (no-code agent platform, incl. a Deep Data Research Agent) and **Oracle Deep Data Security** (ADB Serverless release note 2026-06-30; needs AI Database 26ai and a propagated end-user identity). (chat, 2026-09-07)
...
**NVIDIA's agent stack as pitched in its own decks (Nemotron Master Deck + Agent Toolkit Customer Deck, Aug 2026; read 2026-09-06; NVIDIA-confidential — internal):** Nemotron framed as five capabilities — Reasoning (3.5 Lightning · 3 Super · 3 Ultra) · Vision (3 Nano Omni, Parse 1.1, Nano 2 VL with Efficient Video Sampling) · Retrieval (3 Embed, Llama Nemotron Rerank, Page Elements, Table Structure) · Safety (3.5 Content Safety, Topic Control, Jailbreak Detection) · Speech (Nemotron ASR, Magpie TTS Multilingual, 3 VoiceChat); the **Agent Toolkit** adds **OpenShell** (policy-based secure runtime), **NeMo Switchyard** (task-aware model routing; "Privacy Router" in NemoClaw for OpenClaw), **NeMo Relay** (trajectory capture / observability; "69% inference cost reduction" claim), **NemoClaw** blueprints (always-on agents for OpenClaw, LangChain Deep Agents, Hermes; bring-your-own harness), **Dynamo** (distributed inference), and **AI-Q as a skill**; blueprints AI-Q Deep Research · VSS (with 3D real-time multi-view spatial AI) · RAG · Voice Agent. Customer proofs quoted: CrowdStrike (96% structured-query accuracy, 5× faster investigations, 3× triage accuracy), Harvey (Ultra post-trained in <24 h), H Company, Glean, CodeRabbit; ServiceNow. ⚠ The decks say nothing about availability of these blocks on OCI — that mapping (Model Import, NIM on OKE / Marketplace / AI Quick Actions) comes from the Oracle sources below.
