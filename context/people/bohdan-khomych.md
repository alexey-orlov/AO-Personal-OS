...
## Role & relationship
- Associate Director of R&D at SoftServe; runs the R&D Delivery Unit Alex is product lead for.
- Owns Alex's onboarding: vision input for the strategy session, Oracle intros (via Karsten), bootcamp materials.
- Decision-maker on Oracle cuOpt-POC funding split (whether the client pays for the initial POC) → [softserve/oracle](../areas/softserve/oracle.md).
- Likely the same "Bogdan" who drives the PM-focused Jumpstart extension mandate (spelling varies in transcripts) → [softserve/jumpstart-pm](../areas/softserve/jumpstart-pm.md). (inferred — same name/role, not yet explicitly confirmed in one artifact)
- Oracle-side named point of contact for SoftServe's joint-delivery access requests (SharePoint/Slack, per Ismail 2026-08-18), and the owner of the **Level 2 partnership business case** (with Olya Moroz) up to CEO approval → [softserve/oracle](../areas/softserve/oracle.md).
- Sets and defends Alex's OKRs; carries the top-down R&D KPI restructure downward, including unfunded domain mandates (Energy, Life Sciences) he is openly uncomfortable with.
