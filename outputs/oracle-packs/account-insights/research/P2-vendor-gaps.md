...
2. **Step 8 (cross-account ripple) is empty for all ten** — but not for the reason it first appears. The edges are **already licensable for sales**: PredictLeads sells 359M+ vendor/partner/customer connections by REST API, flat file, webhook and **MCP**, including "who a company sells to"; Sayari exposes customs-derived supplier/buyer endpoints; Bloomberg SPLC quantifies 200k relationships by revenue exposure off the ASC 275 >10% rule. **Every piece is purchasable and nobody has assembled them** — no product traverses an edge to produce an account implication. The scraped graph also lacks magnitude, which is what an implication needs.
3. **Step 7 (map to the seller's own catalog) has no strong implementation anywhere.** The closest four: LinkedIn Account IQ's "why your product is a good fit" (driven by a **free-text product blurb**, not a catalog); Oracle **26B's "product relevancy"** line in the brief and **Content Intelligence's sales battlecards**; Salesforce grounding agents in "product FAQ, case studies, and sales plays"; Introhive inferring cross-service opportunity from the **relationship graph**. Sales-enablement tools (Highspot/Seismic/Showpad) recommend **content** keyed on deal stage, never on an external event. Salesforce Revenue Cloud can pull the real catalog into a chat — but in the **quoting** path, driven by purchase history. The two halves exist and are not wired together.
...
**Correction, and it sharpens the gap rather than closing it: the edges ARE commercially licensable for sales use, today.**
- **PredictLeads Connections Dataset** — detects **vendor, partner, integration and investor** relationships, and explicitly "key customers data **identifies who a company sells to**". Built by crawling customer / partner / integration / case-study / testimonial / portfolio pages, plus "**logo image recognition and OCR** to surface connections that plain text parsing would miss". Coverage: "**359 million or more connections across 61.7 million or more websites, with more than one million new connections added each week**". Delivered by "**REST API, flat files, webhooks, and Model Context Protocol (MCP)**" [T3 predictleads.com]. It names Explorium, MixRank, BuyerCaddy, Ocean.io and TheirStack as peers — so this is a small but real *category*.
- **Sayari** — `Suppliers` and `Buyers` endpoints returning a company entity plus shipment metadata, with faceted breakdowns by country, risk factor and HS code, derived from **customs trade transactions** [T1 documentation.sayari.com]. Packaged for compliance and due diligence, but API-accessible.
- **Veridion, Craft.co** — company knowledge graphs with Search and Match & Enrich APIs, explicitly sold as an embeddable data layer [T1/T3].

**So the honest gap moves, and gets more specific.** It is **not** "the graph doesn't exist", and it is **not** "you can't license it for sales". It is that **no product reasons over those edges to produce an account implication.** PredictLeads sells the edge as a *prospecting filter* ("who already has a connection to us"), not as a *propagation path* ("X had an event → traverse to Y → what does that mean for Y"). Every piece is purchasable; nobody has assembled them.

**What genuinely remains hard:**
1. **Edge quality is heterogeneous and largely unquantified.** Bloomberg quantifies revenue exposure for 200k relationships because it employs analysts and leans on the ASC 275 >10% disclosure. PredictLeads' 359M connections are *scraped from marketing pages* — a logo on a customer page tells you a relationship exists, not that it is 30% of revenue. An implication needs the magnitude; the cheap graph does not carry it.
2. **Coverage cliff on private companies.** The disclosure regime covers public issuers. For a private mid-market account the quantified edge simply does not exist, and Bloomberg's answer — analyst estimates — does not scale to a long-tail book.
3. **Citation difficulty compounds along the path.** "Acme had a bad quarter" is citable to a filing. "Therefore Beta is at risk" is an inference over an edge that may itself be a scraped logo or an analyst estimate. That is exactly the claim type needing the evidence-class labelling of C3 — which no enterprise product ships. **The ripple gap and the confidence gap are the same gap seen twice.**

Worth naming precisely because the tractable subset is small, sourceable and completely unserved on the sell side: public accounts, disclosed >10% customer concentrations, named-customer risk factors, and public contract and tender awards.

### C2. Mapping to the seller's OWN service catalog — **four weak implementations, no strong one**
...
- **Oracle**, two places: the 26B brief includes a "**product relevancy**" section [T1 readiness doc] — a named output field, but with no documented mechanism or catalog binding; and **Content Intelligence** (26B, inside AI Agent Studio) builds knowledge graphs and **sales battlecards** over SharePoint/Slack/Jira/Confluence/Drive/Box content [T1]. That is Oracle's best step-7 mechanism and it is, again, retrieval over collateral.
- **ZoomInfo's "Why Now" skill** — "pairing a **timing thesis** with the specific hooks that make the moment relevant" [T1 release notes]. It gets the *timing* argument right and says nothing about which of your offerings the moment buys.

**The two adjacent categories that look like they solve it, and don't:**
- **Sales enablement (Highspot / Seismic / Showpad).** These do recommend from the seller's own library — but they recommend **content**, keyed on **deal stage and buyer persona**, not on an external event's implication. Highspot's AI "can suggest relevant content based on deal stage and buyer profile", with personalization "mostly… at the 'assembly' level"; Showpad's **PitchAI** "assists with content selection" [T3 comparison write-ups]. The input is where the deal is, never what just happened in the world. (Corporate note: **Seismic announced intent to merge with Highspot in February 2026** [T3] — see `## Unverified`.)
- **Salesforce Revenue Cloud / Agentforce Revenue Management.** Agentforce *is* natively part of Revenue Cloud and can "pull in the product catalog directly in a chat", with "AI-driven suggestions based on purchase history or customer profile" [T3]. So a structured catalog *is* reachable by a Salesforce agent — but in the **quote/commerce** path (Product Catalog, Pricing, Order Management, CLM, Billing), driven by purchase history, not by an implication derived from an external event. The two halves exist in the same platform and are not wired together.

**What does not exist anywhere in the ten vendors + nine direct competitors + these two adjacent categories:** a surface where the seller **loads a structured catalog** (service lines, SKUs, delivery capabilities, qualifications, reference cases, price bands) and the system returns, per implication, **which specific offering it buys and why** — with the offering cited as precisely as the news is.
...
| C-3d | PredictLeads — "Vendor Relationship & Supply Chain Intelligence Tools" (Connections Dataset: vendor/partner/integration/investor + key customers "who a company sells to"; 359M+ connections across 61.7M+ websites, 1M+/week; logo recognition + OCR; REST API / flat files / webhooks / MCP; peers named: Explorium, MixRank, BuyerCaddy, Ocean.io, TheirStack) · https://predictleads.com/blog/vendor-relationship-supply-chain-intelligence-tools/ | T3 |
| C-3e | Sayari — Trade Search: Suppliers & Buyers (company entity + shipment metadata, faceted by country/risk/HS code; customs-derived) · https://documentation.sayari.com/api/guides/trade-search-suppliers-buyers ; Sayari Graph · https://sayari.com/platform/graph/ | T1 |
| C-3f | Veridion (company knowledge graph, Search API, Match & Enrich API, embeddable data layer) · https://veridion.com/data/delivery ; Craft.co Company Data API (GraphQL) · https://docs.craft.co/company-data-api/introduction | T1/T3 |
...
16. **~~Whether any sell-side product licenses SPLC-class relationship data.~~ PARTIALLY CLOSED.** A sell-side relationship-graph *data* category does exist and is API-licensable for prospecting — PredictLeads (plus Explorium, MixRank, BuyerCaddy, Ocean.io, TheirStack), Sayari, Veridion, Craft.co. What remains unverified is (a) whether any of those carry **quantified** exposure (revenue share) rather than binary edges — PredictLeads' scraped-logo provenance suggests not, but this was not confirmed from a spec; and (b) whether Bloomberg SPLC data is licensable outside the terminal via Data License / Enterprise Access Point. Both worth one targeted check.
...
### Additional unverified items from the Part B2 substitutes research

19. **Perplexity — everything.** perplexity.ai/enterprise, /help-center and /hub all return 403 to automated fetch. The Salesforce/HubSpot connectors, "400+ connectors", Deep Research over internal knowledge, structured outputs, scheduling and write-back are **all T2/T3**. Do not build any competitive claim on Perplexity without a manual check.
20. **`help.openai.com` (all articles) returns 403**, so these remain snippet-only: scheduled-task per-plan caps (Go 3 / Plus 5 / Business+Edu 10 / Pro+Enterprise 15), hourly recurrence limit, event-trigger limits (30/hr, 720/day), the 2025-12-17 "connectors → apps" rename, the company-knowledge article, the 2026-02-10 "connect deep research to any MCP or app" change, and **legacy deep research removal 2026-03-26**.
21. **OpenAI's own stated deep-research limitations** (hallucination, confidence calibration, citation reliability) — `openai.com/index/*` not reachable. Unverified.
22. **Whether ChatGPT deep research can run *inside* a scheduled task.** The T1 automations doc lists connected tools, skills and plugins but does not name deep research. Unverified either way.
23. **Whether Agentforce Account Research uses external web/news at all.** `help.salesforce.com` is a JS shell; the T1 newsroom page names only Customer 360 / Data 360 / Slack. The widely-repeated claim that Agentforce briefs include "web updates" is **T2/T3 and unverified**. This is the single most consequential open question in Part B, because it decides whether Salesforce already occupies the whole job or only its first-party half.
24. **Gong AI Briefer scheduling, automation, and Slack/CRM delivery** — the T1 help doc **explicitly does not address them**. Unattended coverage via Gong is unverified.
25. **AlphaSense alerts/watchlist API and Salesforce integration** — press-release claims, **not present in the developer docs**. Unverified.
26. **A ChatGPT↔Salesforce deep-research connector** — HubSpot is documented as "the **first** CRM deep research connector"; no Salesforce equivalent found. Absence of evidence only.
27. **Claims explicitly checked and NOT substantiated — discard rather than repeat:**
    - "34% decrease in prospect research time with AI agents", attributed to the Salesforce newsroom: **all 36 pages of the State of Sales PDF were read; it is not there.**
    - HubSpot's "up to 95% reduction in time reps spend researching accounts" — T3 blog, no methodology.
    - "4x increase in meeting book rates" from research-to-email — T3 SEO content, unattributed.
    - SPOTIO "9%/11% of time on prospecting research" — repeated in T3 blogs; SPOTIO's own report not reached.
    - **LinkedIn State of Sales** — no current edition surfaced; the only figure found (82% vs 49%) is from the **2022** edition via T3 blogs. The series may be discontinued.
    - **Forrester/Gartner hours-per-account benchmarks do not appear to exist.** Both publish *buyer*-side data (70–80% of journey pre-contact; 6–10 stakeholders; 67% prefer rep-free), **not seller research hours**. The Salesforce 16% figure is the only T1 seller-side number available — treat it as the sole anchor.
