...
│   └── gen-heroes.py         regenerates the eight generated hero backgrounds
...
- **All nine heroes hold one visual register** — near-black ground, one soft teal core, thin teal line work, nothing representational, no rendered text. Eight are generated by `tools/gen-heroes.py` (deterministic, re-runnable); `cross-system-erp-qa.jpg` is the reference the rest were matched to. A replacement image has to hold that register: a set that mixes registers reads as assembled from whatever was to hand, and a photographic or generated hero carrying fake text or malformed anatomy is the loudest "AI page" tell on a surface sellers demo live.
