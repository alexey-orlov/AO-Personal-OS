...
| `#/services` | Services, in three screens and the contact block (round 6, `docs/PROVENANCE.md` §21): what SoftServe adds on Oracle — hero, stat band, platform chips · how we engage (`#how-we-engage`: Jumpstart proof of value → Integration → Scale) and who runs it after go-live · how we measure it (`#proof`, the light band) · contact form (`#contact`) |
...
`site/demo/cross-system-erp-qa/` is the third walkthrough — same tour engine, same URL switches, same standards — and the first one that is not a single application. A fictional multi-entity group runs three ERPs (Oracle Fusion Cloud ERP, JD Edwards EnterpriseOne, NetSuite) plus an in-house Oracle Database application and a non-Oracle CRM; the group controller rebuilds one governed model over all five, asks a question in plain English and follows every figure back to the row it came from, a data steward overrules a match the model got wrong, and the same question is re-run under a second role to show the row policy and the masking hold. **Three product surfaces** sit under one neutral workspace switcher, each keeping its own look: *Autonomous AI Lakehouse · Data Studio* (Catalog with five mounted source catalogs and the `GOLD` certified views, Data Load › Live Feed with the model-rebuild job, Analysis), *AI Data Platform · Agent Hub* (the conversational answer surface — saved questions, a result grid with a per-row source badge, Explore / Explain / Code View / Trace, plus Master catalog, column-level Lineage, Auto-populate catalog, Insights and Sessions) and **Mapping review**, a small Redwood app for the steward — the health band, the supplier-match and account-mapping queues, and the decisions log.

- The six steps: **1** rebuild the model in Data Studio (five stages: sync, resolve identities, map accounts, translate and reconcile ledgers, rebuild certified views) → **2** a passive step on the health band — five sources in one model, identities resolved, accounts mapped, ledgers tied, duplicate payments surfaced → **3** ask the cross-system question in Agent Hub and get an answer whose every row names the systems it came from → **4** open the Trace (SQL, the terms resolved from catalog descriptions, the SQL Firewall line, rows and ms) and Explore one row down to the two source records behind it → **5** reject the wrong match in Mapping review with a reason, re-run the resolution, and watch the band, the duplicate list and the answer move together → **6** re-run the same question as a regional analyst — fewer rows, bank and tax columns masked in the database — and publish the answer as a certified view.
- Switches: `?tour=off`, `?ui=clean`, `?state=start|refreshed|fixed|final`, `?app=lakehouse|aidp|review`, `?role=controller|analyst`, `?q=1..10`, and `?panel=` — `trace|explore|code|explain` (the open panel under an answer), `catalog|feeds|analysis` (a Data Studio screen), `home|conversation|insights|mcatalog|apc|lineage|sessions` (an Agent Hub nav item), `matches|accounts|decisions` (a Mapping review tab).
- Every number on screen is computed in the page from `data.js`; `node tools/erp-qa-check.js` reconciles them (267 assertions) and `tools/capture-erpqa-tour.json` replays the whole tour by real clicks as the regression test (`LOGS: none` is the gate).
- Frames and poster: `docs/ASSETS.md` §1; the round: `docs/PROVENANCE.md` §22. `demoPreviewUrl` is still empty — the standalone artifact has not been published yet, so inside the site's own artifact preview the demo button falls back to the relative path, which that host refuses. Everywhere else (a static host, `file://`, a local server) the button works today.
