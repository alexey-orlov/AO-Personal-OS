...
**Hero backgrounds** (`site/assets/img/heroes/`, nine 1920×900 JPEGs): eight are **generated in this repository** by `tools/gen-heroes.py` and carry no third-party rights of any kind — re-running the script reproduces the shipped bytes exactly. The ninth, `cross-system-erp-qa.jpg`, is the generated reference image the other eight were matched to. `heroes.json` carries `file`, `alt` and `focal` only; it deliberately carries no provenance field, because it is served publicly — see §11.1.
...
**All eight were replaced.** `cross-system-erp-qa.jpg` — the one image in the set that was clean, teal and on-brand — was kept unchanged and became the reference register. The other eight were **generated procedurally in this repository**, by `tools/gen-heroes.py` (deterministic Python/Pillow: near-black ground, one soft teal core, thin 1px line work, no representational subject matter, no rendered text, no depicted people or devices), each with a motif drawn from its own product:
