...
- `overview.caseStudies.length === 3`; every card's `descriptor`, `area`, `industry` and `status` equal its product's `overview.caseStudy` values, its `product.name` equals that product's `name`, and a card whose headline value carries no number does not disclaim figures.
