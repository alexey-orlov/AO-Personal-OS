...
- `site.nav` holds **exactly three** items — Products `#/products`, Services `#/services`, Case studies `#/#case-studies` — and `site.navCta` and `site.primaryCta` each carry a non-empty `{ label, route }` (`site.secondaryCta` was retired in round 6 and fails if it returns).
