...
**Round 6 (2026-09-16) — three screens and the contact block** (`PROVENANCE.md` §21): what SoftServe adds on Oracle (hero + stat band) · how an engagement runs and who runs it after go-live (`#how-we-engage`) · how we measure it (`#proof`, the page's one light band) · contact (`#contact`).

| Key | Type |
|---|---|
| `hero` | `{ image: { file, alt, focal }, headline: { accent, rest }, lead, secondParagraph, stats: [{ value, label }], platformsTitle, platforms: [{ name }], cta: { label, route } }` — `image` is the hero background, same contract as `products[].hero.image`. `headline.accent` renders on its own teal line. `platforms[].name` are the four canonical `facets.technology` labels in facet order (checker T3) and render as the "Runs on" chips. `cta` is the page's one button. |
