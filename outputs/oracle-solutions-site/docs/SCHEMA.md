...
| `hero.stack` | `{ ariaLabel, patternsLabel, softserve: { label, items[3] }, platformsLabel }` | The copy of the **built-on stack visual**, this hero's only illustration. Three bands: what you can start with on top (`patternsLabel` is *"Agents and workflows to start with"* — an invitation, not a definition of a taxonomy, and it carries **both halves of the positioning**, §18.9), the SoftServe layer in the middle (`items`, exactly three: *Agentic and data engineering · Evaluation & guardrails · Pilot to production* — the first from `services.whatWeDo.layering[0].body`, which is where *agentic and data engineering* is already written), the Oracle platforms underneath. **The tiles themselves are derived, not written here** — the three pattern tiles from `facets.categories` (chip + glyph) with each pattern's product names as chips in `SITE_CONFIG.productOrder`, the four platform tiles from `facets.technology` (label + glyph). So the visual cannot name a platform in words the rail does not use. `ariaLabel` is the figure's accessible name: it renders `role="img"` with its internals `aria-hidden`, so it is the only way the figure reaches assistive tech — it must describe the bands as they are labelled, including the *agents and workflows* the top band names. |
...
| `twoWays` | `{ eyebrow, title, panels[2] }` | S2. `title` is **one sentence** — *"Start with a product, keep the team that built it."* — because a mid-line period orphans the first word of a two-word second sentence at this size. (Until 2026-09-18 the reason was that the screen set it in uppercase Montserrat, where a period is invisible; the rule outlived its original reason.) It says **product**, not *agent*: the H2 covers all seven and four of them are not a single agent (§18.9). `panels[0].title` is *"Agents and workflows"* for the same reason. Each panel is `{ id, icon, title, body, bullets[3], cta { label, route, direction } }`. **Three bullets each, because the two panels are peers** — same shape, same height, CTAs on one baseline. `icon` must be a key of the `ICONS` registry in `assets/app.js` (the checker reads the registry and fails on a name that is not there). `direction: "down"` selects the `arrowDown` glyph: both CTAs scroll to a screen further down this page. `panels[0].body`'s closing clause — *"at a fixed price where one is published, otherwise scoped per engagement"* — is **load-bearing and stays**: it is the site's one statement of its commercial shape and the guard against implying every product is ready to buy (§18.8 f rejects replacing it with something else). It states that shape **without counting it**: the arithmetic form, *"Four are priced today; three are scoped per engagement"*, was deleted in §18.9 because a census of the catalog invites the reader to subtract. The same clause closes `productsPage.intro`. `panels[1].body` carries the round's one positive breadth statement — *"or build the one your workflow needs"* — sourced to `services.whatWeDo.lead` and `layering[0].body`. |
...
| `caseStudiesIntro` | `{ eyebrow, title, body, ndaLine, cta }` | S5's left rail, and **the whole of it**: head → NDA line → one link. `title` names the two states the cards are in without counting them — *"What we've proven, and what we're proving now."* — because the four are one `measured`, one `modeled` and two `in-preparation`: a title claiming all four are measured is false of three of them, a title counting them is scaffolding (§18.7), and *"What each engagement measures"* was rejected by name as a block title (§18.8, item 6 — it now heads nothing on the home page and `services.proof.engagementsTitle` gave it up too). `body` is **one sentence**: each card prints its own state in its status chip — `Proven` / `Forecast` / `Estimated` — so the rail does not enumerate them (38 words → 14, §18.8). `ndaLine` is one short line, *"Reference calls on request."* **The measurement method is not here at all** — the rail used to reuse `services.proof.lead`, `.stat` and `.footnote`; since §18.8 the method ships only on Services, and `cta` (*How we measure it* → `#/services#proof`) is how the reader gets to it. |
...
Retired, and failed by the checker if they come back — round 7: `services.afterGoLive` (folded into the Scale step), `services.proof` (its measurement argument opens `howWeEngage`, its 81% figure closes the step track's footnote); round 6: `services.whatWeDo`, `services.whySoftServe`, `howWeEngage.ladder` / `ladderRules` / `ladderFootnote` / `howAPovRuns`, `hero.platforms[].short` / `long`, `site.secondaryCta`, `site.dividerLabels`, `shared.ladderColumns`.
...
- Every product's `facet` is one of the four `facets.technology[].id` values — `oci-nvidia` (five products), `oracle-ai-lakehouse` (two); `oracle-ai-data-platform` and `oracle-ai-fusion` match no product today, so the rail does not list them and `?tech=<id>` renders their `emptyState`.
...
- `shared.tagFamilies` covers all three `facets.categories[].id` values and all four `facets.technology[].id` values, and carries both availability badges. `shared.caseStudyStatus` carries exactly `measured`, `modeled` and `in-preparation`, whose chips are exactly `Proven`, `Forecast` and `Estimated`.
- `site.nav` holds **exactly two** items — Products `#/products`, Services `#/services` (Case studies left the header on 2026-09-17) — and `site.navCta` and `site.primaryCta` each carry a non-empty `{ label, route }` (`site.secondaryCta` was retired in round 6 and fails if it returns).
...
- **No surface states a total or names a gap.** `productsPage.count` and `facets.footnote` do not exist; `productsPage.intro`, `productsPage.bottomBlock.heading` / `.body`, `overview.twoWays.panels[0].body`, `overview.catalog.lead` and `overview.catalog.title` carry none of *seven* / *these seven* / *four are priced* / *three are scoped* and none of *so far* / *yet* / *not seeing*.
