...
| `sectionLabels` | map | The standing headings of the visual grammar — the Overview, Technology and POV section titles that are the same on all seven products (`metrics`, `metricsPlanned`, `roi`, `industries`, `scope`, `scopeIn`, `scopeOut`, `moreDetail`, `moreDetailFeatures`, `architecture`, `flow`, `stack`, `notUsed`, `security`, `howItWorks`, `industryCases`, `caseProblem`, `caseSolution`, `outcomes`, `atAGlance`, `factCategory`, `factPlatform`, `factAvailability`, `factPovDuration`, `factPovPrice`, `povLink`, `layerRequired`, `layerOptional`, `directionInbound`, `directionOutbound`, `contacts`, `povHeading`, `povFact*`, `deliverables`, `pricing`, `terms`, `matrix`, `ladder`, `povScopeIn`, `povScopeOut`, `povRollout`, `povPhases`, `povMeasured`). Product-specific headings stay in the product object. `features`, `components` and `integration` were removed with the blocks they headed — the stepper carries the feature bullets and the layered stack carries both of the others. |
...
| `industries` | `[string]` | Keys from the **fixed set of 16** in `VISUAL-GRAMMAR.md` §5. Rendered as icon chips **inside the More-detail disclosure**; the industry tabs (`industryCases`) are what a reader sees above it. `cross-industry` is the reserved honest answer where no vertical list exists. |
| `industriesNote` | string | One line under the chips, inside the disclosure: who this is for. Present on all seven. |
...
| `notUsed` | `[string]` | The honest "not used by this product" line, rendered muted under the accordion. May be empty; the key must exist. |
