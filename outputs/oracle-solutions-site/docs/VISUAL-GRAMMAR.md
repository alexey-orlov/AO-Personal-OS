...
2. **Status chip** — *"Measured in the proof of value"*, *"Modeled in the proof of value"* or *"Proof of value in preparation"*, read from `shared.caseStudyStatus` by the `status` key. It is the element that tells a reader, at a glance, what the numbers below are. **The chip, the eyebrow and the caveat sentence in the story must all say the same word** — a card that says *Measured* at the top and *modeled simulations* four lines down retracts its own headline, and it is the first thing a sceptical customer pulls on.
3. **One or two big metrics with an eyebrow** — the eyebrow reads *Measured*, *Modeled* or *Target outcomes*, one per status, and the checker asserts the pairing. Two is the default, and a third would make the panel a metric row competing with §2.4; **one** is correct where only one real outcome exists, and the row then renders as a single column. A case with no published figure sets a **qualitative outcome statement** of the *"Hours, not quarters"* shape — a turnaround or a coverage claim — never an invented number, and never a restatement of the mechanic: *"One signal"* and *"Evidence-backed"* were the product's own description set at 40px in a numbers slot, which is what a slot filled because it was there looks like.
4. **The story** — two to three sentences: what was done, on what data, with which stack. The last sentence carries the caveat that qualifies the figures; the panel has no footnote row, so that is how rule 2 of this file is satisfied here.
...
| `overview.caseStudy` | present — `null`, or `{ descriptor, area, industry, status, metricsEyebrow, metrics ×1–2, scope ×3, story, ndaLine, downloadLabel }` with a caveat clause inside `story`; no `customer`, no `logo`, no `image`; `metricsEyebrow` agrees with `status` |
...
|---|---|---|
| S1 | **Hero** — `overview.hero` | Full-bleed, two columns: eyebrow, H1 (white lead + a teal accent line of its own), ≤ 45-word lead and two buttons on the left; the **built-on stack visual** on the right — three peer bands (what the products do · SoftServe · Oracle platforms) joined by thin connector lines. The **three-tile** `stat-band` sits directly under it (see *The proof strip* below). |
...
- **Three equal columns**, `repeat(3, minmax(0, 1fr))`, rather than the four-column track the Services strip runs: three tiles in a four-column row leave a column standing empty and the band ends where nothing is.
