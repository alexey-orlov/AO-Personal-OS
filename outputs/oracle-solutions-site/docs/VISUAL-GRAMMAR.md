...
|---|---|---|---|
| **What it does** | **Outlined** chip — transparent ground, 1px border | The product's `categoryChip` — *Deep research* · *Document processing* · *Data analysis & optimization* — with the icon from `shared.tagFamilies.pattern.icons[product.category]`: a magnifier over a graph, a pipeline glyph, a chart | `What it does` |
...
**A flag asserts that the thing exists, so it is set only once it does.** `video: true` with an empty `videoUrl` used to put a `Demo` badge and a facet count on two products whose only demo was a panel saying the recording was being prepared: a prospect filtering on *Demo available* and a seller demoing that filter both hit a dead end. The flag now flips when the recording (or the interactive walkthrough) lands, not in anticipation of it.

**Placement.** In the hero, the row splits: pattern and technology chips at the left, availability badges at the **right end**. Below 768px both wrappers dissolve and the badge flows as the **last chip of the run** rather than dropping to a line of its own — a lone badge on a fourth row reads as an orphan and pushes the value proposition below the fold. On a products-page tile the badges sit **top-right of the image band**, where the availability chip used to; the platform label keeps the top-left. On the home page's catalog rows (§9, S3) there is no image band, so the badges sit **inline beside the product name**, scaled down to `.6875rem`. Those rows name **no platform at all** — the column a row sits in already says what the product does, and the platform is on the product's own page, one click away. The tile variant swaps the teal tint for a black scrim so it stays legible over photography, but keeps the 1px teal border and the teal label — it has to stay recognisable as the same family.
...
Services is built from the home page's components rather than its own; the reasoning is in `PROVENANCE.md` §21, the layout QA and the short headings in §21.7.

| Screen | Component | Rule |
|---|---|---|
| S1 — what SoftServe adds on Oracle | product hero over `services.jpg` + `stat-band` | The copy sits in the product hero's own `.product-hero-copy` column (1.25rem rhythm, one 44rem measure for lead and subline). The H1 is a display line, not a sentence — two to four words, the teal accent on its own line above the white rest — set at the **home H1's scale** (`clamp(2.25rem, 3.9vw, 3.75rem)`), not the product title's, and at `min(2rem, 8.6vw)` below 480 px so each half keeps its line down to 320. The four platforms render as the product hero's "Runs on" chips under a small label. One filled button. The stat band shares the home strip's three-figure geometry (§9). |
| S2 — how we engage, then after go-live | `home-head` + `ladder3` + footnote; a second `home-head` with a dim eyebrow + `ways` | The step names are the home page's. The track runs full width here, so steps read at body size (title 1.25rem, body `--fs-body`) and the track turns vertical **below 900 px**, not at the home's 720. Teal lands on the eyebrow and the first step's dot only: the after-go-live marks are neutral. The two panels are peers with no button — the contact block is the page's one ask. |
| S3 — how we measure it | `light-band` | The page's only light band: the discipline left; right, the one figure set as the band's anchor (`clamp(2.75rem, 4.4vw, 4rem)`) with a regular-weight label, its caveat, and the link to the case studies — an internal route with the arrow glyph, set inline in a paragraph with a text underline so a wrapped label keeps its arrow after the last word. |
