...
|---|---|---|
| S1 | **Hero** — `overview.hero` | Full-bleed, two columns: eyebrow, H1 (white lead + a teal accent line of its own), ≤ 45-word lead and two buttons on the left; the **built-on stack visual** on the right — three peer bands (what the products do · SoftServe · Oracle platforms) joined by thin connector lines. The four-stat `stat-band` sits directly under it. |
